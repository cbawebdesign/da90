...
const routes: RoutesWithContent = [
  ...
  // Loads te main window (navigator) together with the localized content 
  {
    ...
    children: [
      ...
      // Donate
      { path: 'hello', loadChildren: () => import('../pages/donate/donate.module').then(m => m.DonateModule) },
      ...
      { path: 'admin', loadChildren: () => import('../pages/admin/admin.module').then(m => m.AdminModule) },
       ...
       // Login dialog
      { path: 'login', loadChildren: () => import('../dialogs/login/login.module').then(m => m.LoginModule), canActivate: [ DialogLoader, LogoutLinkObserver ] },
